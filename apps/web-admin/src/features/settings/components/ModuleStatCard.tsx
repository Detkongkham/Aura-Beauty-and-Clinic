...
export type ModuleStatTone = 'primary' | 'info' | 'success' | 'accent' | 'warning';

const TONE: Record<ModuleStatTone, { chip: string; bar: string; ring: string }> = {
  ...
  accent: { chip: 'bg-accent-soft text-accent', bar: 'bg-accent', ring: 'border-accent/50 ring-accent/30' },
...
/**
 * Module-Management-only stat tile (near-copy of `features/queue/QueueStatCard.tsx`,
 * per the project's per-feature stat-card convention): tone-tinted icon chip +
 * label/figure + accent bar. Motion is baked in — CardCount convention — fade +
 * rise on mount (staggered by `index`), hover lift + press dip when interactive,
