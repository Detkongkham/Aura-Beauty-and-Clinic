...
export type QueueStatTone = 'warning' | 'info' | 'primary' | 'success' | 'danger' | 'neutral';

const TONE: Record<QueueStatTone, { chip: string; bar: string; ring: string }> = {
...
interface QueueStatCardProps {
  ...
  label: string;
  value: ReactNode;
  /** Optional short supporting line under the figure. */
  hint?: string;
  /** Change vs a baseline, e.g. "+3" / "−2m"; `good` colours it. */
...
/**
 * Compact queue-only stat tile — tone-tinted icon chip on the left, label + figure
 * stacked on the right, with a thin tone accent bar. Clickable tiles act as lane
 * filters (aria-pressed).
 *
