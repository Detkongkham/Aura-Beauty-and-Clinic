...
import { useChatReports, useReviewChatReport } from './messaging.api';

type StatusFilter = 'PENDING' | 'REVIEWED' | 'ACTIONED' | undefined;

const STATUS_VARIANT: Record<'PENDING' | 'REVIEWED' | 'ACTIONED', NonNullable<BadgeProps['variant']>> = {
  PENDING: 'warning',
  REVIEWED: 'info',
  ACTIONED: 'danger',
};
...
export function ChatModerationPage() {
  ...
  const reports = useChatReports(status);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-1 rounded-xl border border-border bg-card p-1">
        {(['PENDING', 'REVIEWED', 'ACTIONED', undefined] as StatusFilter[]).map((s) => (
          <button
            key={s ?? 'ALL'}
            type="button"
            onClick={() => setStatus(s)}
          ...
          >
            {s ? t(`chatModeration.status.${s}`) : t('chatModeration.status.ALL')}
          </button>
        ))}
      </div>
              ...
              <li key={r.id} className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-foreground">{r.reportedByName}</span>
                    <Badge variant={STATUS_VARIANT[r.status]}>{t(`chatModeration.status.${r.status}`)}</Badge>
                  </div>
