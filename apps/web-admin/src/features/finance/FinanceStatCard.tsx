...
export type FinanceStatTone = 'primary' | 'success' | 'warning' | 'danger' | 'neutral';

const FINANCE_STAT_TONE: Record<FinanceStatTone, { chip: string; bar: string; ring: string }> = {
...
interface FinanceStatCardProps {
  ...
  index?: number;
  /** Turns the tile into a filter toggle. */
  onClick?: () => void;
  active?: boolean;
  /** Pulsing dot on the icon chip — used to flag figures that need attention. */
  pulse?: boolean;
}

/**
 * Finance overview tile — same anatomy as `InventoryStatCard` / `PricingStatCard`
 * (tone-tinted icon chip + left accent bar) so the Finance module reads as part
 * of the same admin, per the CardCount motion convention: every tile fades +
 * rises on mount staggered by `index`, interactive tiles lift on hover and dip
 * on press, all suppressed under `prefers-reduced-motion`.
 */
export function FinanceStatCard({
  ...
  const c = FINANCE_STAT_TONE[tone];
  const className = cn(
