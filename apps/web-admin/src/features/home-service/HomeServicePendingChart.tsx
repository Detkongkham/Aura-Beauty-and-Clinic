...
const OPEN_STATUSES: HomeServiceTripView['status'][] = [
  'MATCHING',
  'ASSIGNED',
  'EN_ROUTE',
  'ARRIVED',
...
/** Not-yet-completed trips per scheduled hour — companion to {@link HomeServiceThroughputChart}. */
export function HomeServicePendingChart({ trips }: { trips: HomeServiceTripView[] }) {
  const { t } = useTranslation();

  const { buckets, max, busiest } = useMemo(() => {
    const byHour = new Map<number, number>();
    for (const trip of trips) {
      if (!OPEN_STATUSES.includes(trip.status)) continue;
      const h = new Date(trip.startAt).getHours();
  ...
  if (buckets.length === 0) {
    return (
      <div className="flex h-full min-h-[120px] flex-col items-center justify-center gap-2 text-center">
        ...
        <p className="text-sm font-medium text-foreground">{t('homeServiceDispatch.noPending')}</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between">
        <p className="text-2xs font-medium text-muted-foreground">{t('homeServiceDispatch.pendingThroughput')}</p>
        {busiest != null ? (
      ...
      <div className="mt-2 flex h-20 gap-1">
        {buckets.map((b) => (
          <div key={b.hour} className="flex h-full flex-1 flex-col items-center justify-end gap-1">
            <span
              title={`${HH(b.hour)}:00 — ${t('homeServiceDispatch.pendingCount', { n: b.count })}`}
              className={cn(
                'w-full rounded-t-sm bg-warning transition-[height] duration-500 ease-out motion-reduce:transition-none',
                b.count === 0 && 'bg-border',
              )}
